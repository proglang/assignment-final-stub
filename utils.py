...
def repr_BoolOp(self):
    return "BoolOp(" + repr(self.values[0]) + " " + repr(self.op) + " " + repr(self.values[1]) + ")"
...
class Let(expr):
    ...
    def __repr__(self):
        return (
            "Let(" + repr(self.var) + "," + repr(self.rhs) + "," + repr(self.body) + ")"
        )
...
class FunctionType:
    ...
    def __repr__(self):
        return (
            "FunctionType(" + repr(self.param_types) + "," + repr(self.ret_type) + ")"
        )
...
def compile(compiler, compiler_name, type_check_P, type_check_C, program_filename):
    program_root = program_filename.split(".")[0]
    with open(program_filename) as source:
        program = parse(source.read())
